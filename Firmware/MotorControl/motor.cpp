...
void Motor::DRV8301_setup() {
    ...
    static const float kTripMargin =
...
bool Motor::check_DRV_fault() {
    ...
    GPIO_PinState nFAULT_state = HAL_GPIO_ReadPin(
        gate_driver_config_.nFAULT_port, gate_driver_config_.nFAULT_pin);
...
float Motor::get_inverter_temp() {
    ...
    return horner_fma(normalized_voltage, thermistor_poly_coeffs,
                      thermistor_num_coeffs);
}
...
float Motor::effective_current_lim() {
    ...
    if (axis_->motor_.config_.motor_type == Motor::MOTOR_TYPE_GIMBAL) {
        current_lim =
            std::min(current_lim, 0.98f * one_by_sqrt3 * vbus_voltage);
    } else {
        current_lim = std::min(
            current_lim, axis_->motor_.current_control_.max_allowed_current);
    }
    // Thermal limit
...
/* The motor coil can be considered as an LR circuit with the target that we
have to measure phase resistances and inductances. The I controller, converts
this circuit to a basic RLC circuit with constant voltage input, the constant
voltage input and capacitance values depends on the the kI.*/
...
bool Motor::measure_phase_resistance(float test_current, float max_voltage) {
    ...
    static const int num_test_cycles =
        static_cast<int>(3.0f / CURRENT_MEAS_PERIOD);  // Test runs for 3s
...
bool Motor::update(float current_setpoint, float phase, float phase_vel) {
    ...
    if (config_.motor_type == MOTOR_TYPE_HIGH_CURRENT) {
        ...
        if (!FOC_voltage(0.0f, current_setpoint, pwm_phase)) return false;
    } else {
